...
four features Claude Code ships that the Boss says to leave switched
off: agents it dispatches on its own initiative, a background view
that hides as much as it shows, subagents spawning further subagents
until nobody can say what ran, and agent teams, switched off anyway
because experimental means nobody promised the shape holds from one
version to the next.
