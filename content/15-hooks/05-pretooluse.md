...
That is a different power from the pair before it. `SessionStart` and
`SubagentStart` can only add words. `PreToolUse` can say no.
...
Next: what fires the instant that same tool call has already
finished, rather than the moment before it runs.
