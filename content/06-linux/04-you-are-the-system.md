...
*Packages* showed the terminal replacing apps: `diff` instead
...
That is Linux. Agentic AI is next, and it leaves the machine in front of
you for a while — what a model actually is, what a harness is, why the
two are separate things, and what each one costs to run.
