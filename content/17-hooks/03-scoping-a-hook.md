...
The course has made this point before, in other files. `CLAUDE.md` and
`rules/` are read by whoever launches from that folder. An agent or a
skill sits where only the sessions that need it will find it. A hook
now joins them, with the same question attached: not *would this
help*, but *who exactly should this reach, and no further*.
