...
not what each mode actually is — that is next — but how you move a
session between them once it is already running.
...
Which mode is worth that treatment is its own question, and the next
article is what there is to choose between.
