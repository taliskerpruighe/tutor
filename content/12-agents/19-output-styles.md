...
The six fields named in *The fields that matter* live in a definition
file and configure one agent — a separate identity you launch with
`--agent` or hand a job to. An output style is not part of that file at all. It
...
to change, so the setting is never read.
