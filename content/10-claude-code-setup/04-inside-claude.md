...
else in the directory has that reach. It is the most important file you
will own.
...
`agents/` and `skills/` hold what you build later in this course —
custom agents, and the skills you write for them. Do not try to absorb
either now; each gets a part of its own.
...
That is one directory, in one place.
