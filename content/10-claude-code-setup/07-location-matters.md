...
That is Claude Code Setup — installed, found inside `.claude`, and
placed where the right session finds it. Instructions is next: the
`CLAUDE.md` file every session reads before it does anything, how to
write one that earns the room it costs, and the rule for instructions
that only apply sometimes, kept apart from `CLAUDE.md` until an `@`
import invites it in.
