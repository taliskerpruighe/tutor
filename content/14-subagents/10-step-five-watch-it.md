...
**What each one returned.** That it finished is not the point. What
came back is. A reader returning three events for a document with
nine dates in it has gone wrong quietly, and it is the only place you
can catch it, because by the next stage it is one line in a merged
list.
