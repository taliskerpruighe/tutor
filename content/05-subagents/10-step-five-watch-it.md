...
order: 10
summary: Read a chain while it runs, fix it in the skill that got it wrong, and the course is done.
keywords: [exercise, watch, debug, legible, fix, iterate, corrections, party tricks, roadmap, tutor]
---
...
You started at a black window with a blinking cursor. You now have the six
Party Tricks, and — more to the point — you can build. An agent, with
the right model on it. A skill, in your own words, from your own
examples, that fires when you type a slash. And a chain: several of
both, in an order you chose, doing a whole job while you read something
else.
...
What is not here yet is in *About this wiki* — connecting Claude Code to
the rest of your machine, and then getting it to work when you are not
sitting there. Both of those assume everything you have just done.

And the course does not close when you shut the reader. Anything in here
you want said again, in a session rather than on a page, ask for:

```
/tutor what does effort actually change
```

Go and build something you will use.
